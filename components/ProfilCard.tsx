...
const ProfilCard: React.FC<ProfilCardProps> = ({ profil, index }) => {
  ...
  // Sécurisation de l'ID public pour l'affichage
  const displayId = profil?.publicId?.split('-').pop()?.toUpperCase() || 'REF_PENDING';
  
  // Sécurisation du récit pour le résumé
  const storyPreview = (profil?.reformulated_story || profil?.raw_story || "Récit en cours d'indexation...")
    .substring(0, isLarge ? 280 : 150);
          ...
          <div className="flex flex-col sm:flex-row gap-12 items-start">
            ...
            <div className="space-y-6 flex-1">
              <div className="space-y-2">
                ...
                <div className="dymo-label text-[9px]">{profil?.usual_place?.split(',')[0] || "Bruxelles"}</div>
              </div>
            </div>
          </div>
